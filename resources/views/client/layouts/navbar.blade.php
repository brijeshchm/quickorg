...
function filterStickyCities(q) {
    ...
    stickyCityTimeout = setTimeout(async () => {
        try {
            const r = await fetch(`https://api.quickdials.org/api/website/getCityList?city=${encodeURIComponent(q)}`);
...
async function fetchStickySuggestions(q) {
    try {
        const res  = await fetch(`https://api.quickdials.org/api/website/get-keyword-list?keyword=${encodeURIComponent(q)}`);
