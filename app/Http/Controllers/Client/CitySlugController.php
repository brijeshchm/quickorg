...
class CitySlugController extends Controller
{
    ...
    private function fetchData(string $city, string $keyword): ?array
    {
        try {
            $response = Http::timeout(10)
                ->withoutVerifying()->get('https://api.quickdials.org/api/website/city/keyword', [
                    'city'    => $city,
                    'keyword' => $keyword,
                ]);
    ...
    private function businessOwnersData(): ?array
    {
        try {
                $res = Http::timeout(10)->withoutVerifying()
                    ->get('https://api.quickdials.org/api/website/business-owners');
    ...
    private function cityExists(string $city): bool
    {
        ...
        try {
            $response = Http::timeout(5)
               ->withoutVerifying()->get('https://api.quickdials.org/api/website/checkCity', ['city' => $city]);			 
    ...
    private function serviceExists(string $slug): bool
    {
        ...
        try {
            $response = Http::timeout(5)
               ->withoutVerifying()->get('https://api.quickdials.org/api/website/getKeyword', ['keyword' => $slug]);
    ...
    private function fetchCityData(string $city=null)
    {
        try {
             $response = Http::timeout(5)
               ->withoutVerifying()->get('https://api.quickdials.org/api/website/getCityList', ['city' => $city]);
    ...
    private function fetchBusinessData(string $slug=null)
    {
        ...
        try {
                $res = Http::timeout(10)->withoutVerifying()
                ->get('https://api.quickdials.org/api/website/business-details', [
                    'business_slug' => $slug,
                ]);
  ...
  $gallery = is_array($b['gallery'] ?? null)
    ...
    $keywordList = Cache::remember('keyword_list', 3600, function () {
            try {
                $res = Http::timeout(10)
                    ->get('https://api.quickdials.org/api/website/get-keyword-list');
